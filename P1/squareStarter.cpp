...
unsigned char* loadImage(int& img_w, int& img_h){
   ...
   for (int i = img_h-1; i >= 0; i--) {
       for (int j = 0; j < img_w; j++) {
           ppmFile >> red;
           img_data[i*img_w*4 + j*4 + 0] = clamp(red*brightness, 0, 255);
           ppmFile >> green;
           img_data[i*img_w*4 + j*4 + 1] = clamp(green*brightness, 0, 255);
           ppmFile >> blue;
           img_data[i*img_w*4 + j*4 + 2] = clamp(blue*brightness, 0, 255);
           img_data[i*img_w*4 + j*4 + 3] = alpha;
       }
   }
...
void mouseClicked(float m_x, float m_y){   
   ...
   bool inside = true;
   bool last_side = vee(lines[0].normalized(), clicked_mouse) > 0;
   for (int i = 0; i < 4; i++) {
       bool curr_side = vee(lines[i].normalized(), clicked_mouse) > 0;
   ...
   float corner_dist1 = vee(clicked_mouse, p1).magnitude();
   float corner_dist2 = vee(clicked_mouse, p2).magnitude();
   float corner_dist3 = vee(clicked_mouse, p3).magnitude();
   float corner_dist4 = vee(clicked_mouse, p4).magnitude();
   float corner_dists[] = { corner_dist1, corner_dist2, corner_dist3, corner_dist4 };
   for (int i = 0; i < 4; i++) {
       if (abs(corner_dists[i]) <= 0.01) {
           at_corner = true;
       }
   }

   bool at_edge = false;
   float edge_dist1 = vee(clicked_mouse, l1.normalized());
   float edge_dist2 = vee(clicked_mouse, l2.normalized());
   float edge_dist3 = vee(clicked_mouse, l3.normalized());
   float edge_dist4 = vee(clicked_mouse, l4.normalized());
   float edge_dists[] = { edge_dist1, edge_dist2, edge_dist3, edge_dist4 };
   for (int i = 0; i < 4; i++) {
       if (abs(edge_dists[i]) <= 0.01) {
           at_edge = true;
       }
   }
   
   do_translate = inside; // if mouse point in polygon (we have lines l1-l4)
   do_rotate = !inside && !at_corner && at_edge;   // edges (if distance from an edge is like < 0.6 or smth)
   do_scale = !inside && at_corner;    // corners (if distance from a corner is like < 0.6 or smth)
...
void mouseDragged(float m_x, float m_y){
   ...
   Line2D cross_1 = vee(p1, p3);
   Line2D cross_2 = vee(p2, p4);
   Point2D rect_origin = wedge(cross_1, cross_2);

   Line2D start_line = vee(rect_origin, clicked_mouse);
   Line2D new_line = vee(rect_origin, cur_mouse);
   ...
   if (do_scale){
      float mouse_dist = vee(rect_origin, cur_mouse).magnitude();
      float prev_dist = vee(rect_origin, clicked_mouse).magnitude();
      rect_scale = mouse_dist / prev_dist; //the scale will grow or shrink based on the mouse movements
   }
   
   if (do_rotate){
      clicked_angle = acos(dot(start_line.normalized(), new_line.normalized()));
      if (vee(cur_mouse, start_line.normalized()) > 0) {
...
//TODO: Reset the square's position, orientation, and scale
void r_keyPressed(){
...
//Rotate the image when the t key is pressed
void t_keyPressed() {
    ...
    Dir2D disp = rect_pos - origin;
    translate = Translator2D(disp);
    rotate = Rotator2D(0.7853, rect_pos);
...
//Rotate the image when the g key is pressed
void g_keyPressed() {
    cout << "The 'g' key was pressed" << endl;
